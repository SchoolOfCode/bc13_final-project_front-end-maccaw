describe("visits the home page and then navigates to the listings page", () => {
  it("passes", () => {
    ...
    cy.url({timeout: 10000}).should("be.equal", "http://localhost:3000/listings");
...
describe("visit homepage, navigates to log-in, inputs details. and checke if logs in", () => {
  it .only("passes", () =>{
    // cy.session('login', ()=>{
    ...
    cy.get(".Login_form-email-input__3q0BK > .Login_input__r_1vS").type(
      "amina@gmail.com");
    cy.get('.Login_form-password-input__XOMuN > .Login_input__r_1vS').type('password')
